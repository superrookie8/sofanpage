import type { CompetitionMetadata } from "@/features/games/competition";
export type LegacySchedule = CompetitionMetadata & {
	_id: string;
	season: string;
	date: string;
	opponent: string;
...
type SpringSchedule = CompetitionMetadata & {
	id: string;
	season: string;
	date: string;
	time: string;
...
/** Absent fields stay absent for old-client updates; explicit empty optionals remain clearable. */
export function scheduleMetadata(value: CompetitionMetadata): CompetitionMetadata {
	const result: CompetitionMetadata = {};
	for (const key of ["competitionKey", "competitionName", "competitionKind", "editionLabel", "teamType", "ourTeamName", "venueType", "venueName", "stadiumId", "stage"] as const) {
		if (Object.prototype.hasOwnProperty.call(value, key)) Object.assign(result, { [key]: value[key] });
	}
	return result;
}
...
export function legacyScheduleRequest(value: LegacySchedule) {
	return {
		...((value.competitionKey === "legacy-special" && (!value.teamType || !value.ourTeamName)) ? {} : scheduleMetadata(value)),
		season: value.season,
		date: value.date,
		time: value.time,
		opponent: value.opponent,
