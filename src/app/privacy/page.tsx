...
const PRIVACY_SECTIONS: readonly LegalSection[] = [
	...
	},
	{
		...
		content: <><p>직관일지, 방명록과 일반 회원가입의 신규 접근 및 작성 기능은 현재 차단되어 있습니다. 다만 과거 운영 과정에서 입력된 정보가 남아 있을 수 있습니다.</p><ul><li><strong>과거 일반 가입 계정:</strong> 이메일, 닉네임과 단방향 해시로 저장된 비밀번호</li><li><strong>직관일지:</strong> 동행인, 좌석, 관람 기록, 메모와 사진 등</li><li><strong>방명록:</strong> 작성자명, 메시지와 사진 등</li></ul><p>이 정보도 열람·정정·삭제 요청 대상입니다. 자동 탈퇴나 연결된 사진의 일괄 삭제 기능은 현재 제공하지 않으므로 운영자가 요청을 확인하여 수동 처리합니다.</p></>,
	},
	{
		id: "retention",
		title: "보유기간",
		content: <ul><li>활성 계정, 프로필과 아케이드 점수: 이용자의 삭제 요청을 처리할 때까지</li><li>로그인 세션 쿠키: 생성 후 최대 24시간</li><li>현재 접근이 중단된 일반 가입 계정·직관일지·방명록 정보: 이용자의 삭제 요청을 처리할 때까지</li><li>관계 법령에 따라 보존할 의무가 생기는 경우: 해당 법령이 정한 기간</li></ul>,
	},
	{
		id: "processors",
		title: "처리위탁과 외부 서비스",
	...
	},
	{
		id: "destruction",
		title: "삭제와 파기",
		content: <p>삭제 요청이 확인되면 운영자는 서비스 데이터베이스의 대상 정보를 삭제하고, 별도 이미지가 있는 경우 저장소의 연결 파일을 확인해 수동으로 삭제합니다. 전자 파일은 복구하기 어려운 방식으로 삭제합니다. 백업이나 관계 법령상 보존이 필요한 정보가 있다면 해당 목적에만 분리하여 보관한 뒤 기간이 끝나면 삭제합니다.</p>,
	},
	{
		id: "rights",
