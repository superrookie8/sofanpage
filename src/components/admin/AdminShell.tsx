...
import { adminAccessFailure } from "@/lib/admin/access-state";
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
	...
	useEffect(() => {
		...
		fetch("/api/admin/session", { cache: "no-store" }).then((response) => {
			if (!active) return;
			if (adminAccessFailure(response.status, sessionStatus) === "login") {
				router.replace(`/login?callbackUrl=${encodeURIComponent(pathname)}`);
				return;
			}
			if (!response.ok) {
				setNeedsReauth(response.status === 401);
				setAuthError(response.status === 401 ? "로그인 인증을 다시 확인해야 합니다. 재시도하거나 다시 로그인해주세요." : response.status === 403 ? "관리자 권한이 필요한 화면입니다. 현재 로그인한 계정에는 관리자 권한이 없습니다." : "관리자 권한을 확인하지 못했습니다. 잠시 후 다시 시도해주세요.");
				return;
			}
			setAuthChecked(true);
		}).catch(() => { if (active) setAuthError("관리자 서버에 연결하지 못했습니다."); });
		return () => { active = false; };
	}, [pathname, router, retry, sessionStatus]);
	...
	if (authError) return <main className="admin-loading"><h1>관리자 접근 확인</h1><p role="alert">{authError}</p><button onClick={() => setRetry((value) => value + 1)}>권한 다시 확인</button>{needsReauth && <button onClick={() => signOut({ callbackUrl: `/login?callbackUrl=${encodeURIComponent(pathname)}` })}>다시 로그인</button>}<Link href="/">팬페이지로 돌아가기</Link></main>;
		...
		<div className="admin-shell">
			...
			<div className="admin-workspace">
				<header className="admin-topbar">
					...
					<div className="admin-server-status"><i /><span><strong>ADMIN SESSION</strong><small>관리자 권한 확인됨</small></span></div>
				</header>
